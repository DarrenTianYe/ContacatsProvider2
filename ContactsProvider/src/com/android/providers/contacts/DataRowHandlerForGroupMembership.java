...
public class DataRowHandlerForGroupMembership extends DataRowHandler {
    ...
    public long insert(SQLiteDatabase db, TransactionContext txContext, long rawContactId,
        ...
        * @{
        */
        ContactProxyManager.getInstance(mContext).onDataUpdate(rawContactId,values,GroupMembership.CONTENT_ITEM_TYPE);
        /**
        * @}
        */
    ...
    public int delete(SQLiteDatabase db, TransactionContext txContext, Cursor c) {
        ...
        */
        ...
        ContactProxyManager.getInstance(mContext).onDataUpdate(rawContactId, tmp,
    ...
    */
    public void appendSearchableData(SearchIndexManager.IndexBuilder builder) {
        String groupRowId = builder.getString(Data.DATA1);
        if (TextUtils.isEmpty(groupRowId)) {
            return;
        }
        int groupRowIdInt = -1;
        try {
            groupRowIdInt = Integer.parseInt(groupRowId);
        } catch (NumberFormatException e) {
            e.printStackTrace();
            return;
        }
        String groupTitle = mDbHelper.getGroupTitle(groupRowIdInt);
        if (TextUtils.isEmpty(groupTitle)) {
            return;
        }
        builder.appendContent(groupTitle);

        int N = groupTitle.length();
        for (int i = 0; i < N; ++i) {
            builder.appendToken(groupTitle.substring(i));
        }
    }
    /**
    * @}
