...
import com.sprd.providers.contacts.DataRowHandlerForWebsite;
...
public class ContactsProvider2 extends AbstractContactsProvider
    ...
    /**
     * add by xuhong.tian store the default group value
     */
    ...
    private interface DataContactsQuery {
        ...
        public static final String[] PROJECTION = new String[] {
            ...
            ContactsColumns.CONTACAT_MODE_TYPE,
            ContactsColumns.CONTACT_IMPORT_TYPE
     ...
     * @{
     */
    public static final String DEFAULT_ACCOUNT_TYPE = "sprd.com.android.account.phone";
    /**
     * @}
     */
    ...
    private static final ProjectionMap sContactsColumns = ProjectionMap.builder()
             ...
             * add by xuhong.tian
             */
            .add(Contacts.CONTACT_IMPORT_TYPE)
            .add(Contacts.CONTACAT_MODE_TYPE)
            /**
             * add by xuhong.tian
             */
    ...
    private static final ProjectionMap sRawContactColumns = ProjectionMap.builder()
            ...
            .add(RawContacts.RAW_CONTACT_IMPORT_TYPE)
            .add(RawContacts.RAW_CONTACT_MODE_TYPE)
    ...
    private static final ProjectionMap sPhoneLookupProjectionMap = ProjectionMap.builder()
            ...
            .add(Contacts.CONTACAT_MODE_TYPE, "contacts_view." + Contacts.CONTACAT_MODE_TYPE)
            .add(Contacts.CONTACT_IMPORT_TYPE, "contacts_view." + Contacts.CONTACT_IMPORT_TYPE)
    ...
    private static final ProjectionMap sGroupsProjectionMap = ProjectionMap.builder()
            ...
            .add(Groups.SYNC3)
            .add(Groups.SYNC4)
            .add(Groups.GROUP_NUMBER)
            .add(Groups.GROUP_CURRENT_MODE)
            .add(Groups.GROUP_BY_IMPORT_TYPE)
            .add(Groups.GROUP_LOCATION)
            .add(Groups.GROUP_DEFAULT)
            .add(Groups.GROUP_TYPE)
            .add(Groups.GROUP_ZONE_NAME)
            .add(Groups.GROUP_ZONE_ID)
            .add(Groups.GROUP_ID)
            .add(Groups.GROUP_RESPONSE_FLAG)
            .build();
    ...
    private boolean initialize() {
        ...
        // To initialize ContactAccountManager
        ContactAccountManager.getInstance(getContext());

        Log.d(TAG, "Initialize watchdog service");
    ...
    protected void performBackgroundTask(int task, Object arg) {
        ...
        switch (task) {
            ...
            case BACKGROUND_TASK_UPDATE_ACCOUNTS: {
                ...
                /**
                 * SPRD:
                 * To update ContactAccountManager
                 * @{
                 */
    ...
    /**
     * SPRD:
     *  modify this method when merge 4.1 to 4.3
     *  Original Android code:
     *  flushTransactionalChanges();
     * @{
     */
    private void flushTransactionalChanges() {
        ...
        for (long rawContactId : mTransactionContext.get().getInsertedRawContactIds()) {
            ...
            long contactId = mAggregator.get().onRawContactInsert(mTransactionContext.get(), db,
                    rawContactId);
            ContentValues proxyValues = ContactProxyManager.getInstance(getContext())
                    .insert(rawContactId,
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                if (!ContactsContract.isProfileId(rawContactId)) {
                    ContactProxyManager.getInstance(getContext()).update(rawContactId);
                }
            ...
            final Set<Long> changedRawContacts = mTransactionContext.get()
    ...
    */
    private long insertGroup(Uri uri, ContentValues values, boolean callerIsSyncAdapter) {
        ...
        if (TextUtils.isEmpty(mValues.getAsString(RawContacts.ACCOUNT_NAME))) {
            Log.e(TAG, "set to default account");
            String accountName = getQueryParameter(uri, RawContacts.ACCOUNT_NAME);
            String accountType = getQueryParameter(uri, RawContacts.ACCOUNT_TYPE);
            if (!TextUtils.isEmpty(accountName) && !TextUtils.isEmpty(accountType)) {
                mValues.put(RawContacts.ACCOUNT_NAME, accountName);
                mValues.put(RawContacts.ACCOUNT_TYPE, accountType);
            } else if (getDefaultAccount() != null) {
                mValues.put(RawContacts.ACCOUNT_NAME, getDefaultAccount().name);
                mValues.put(RawContacts.ACCOUNT_TYPE, getDefaultAccount().type);
            } else {
                mValues.put(RawContacts.ACCOUNT_NAME, "phone");
                mValues.put(RawContacts.ACCOUNT_TYPE, "sprd.com.android.account.phone");
            }
        }
        ...
        Account account = new Account(
                mValues.getAsString(RawContacts.ACCOUNT_NAME),
                mValues.getAsString(RawContacts.ACCOUNT_TYPE));
        ...
        if (!callerIsSyncAdapter) {
            ContentValues proxyValues = ContactProxyManager.getInstance(getContext()).insertGroup(
                    result, mValues, account);
            if (proxyValues != null) {
                Set<String> keys = proxyValues.keySet();
                StringBuilder sb = new StringBuilder();
                boolean init = true;
                ...
                db.execSQL(
                        "UPDATE " + Tables.GROUPS +
                                " SET " +
                                sb.toString() +
                                " WHERE " + Groups._ID + " = " + result
                        );
            }
        }

        if (!callerIsSyncAdapter && isFavoritesGroup) {
            // If the inserted group is a favorite group, add all starred raw
            // contacts to it.
    ...
    public int deleteGroup(Uri uri, long groupId, boolean callerIsSyncAdapter) {
        ...
        try {
            if (callerIsSyncAdapter) {
                ...
                ContactProxyManager.getInstance(getContext()).removeGroup(groupId);
    ...
    */
    private int deleteContact(long contactId, boolean callerIsSyncAdapter) {
        ...
        Cursor c = db.query(Tables.RAW_CONTACTS, new String[] {
            RawContacts._ID
        },
        ...
        try {
            ...
            while (c.moveToNext()) {
                ...
                deleteRawContact(rawContactId, contactId, callerIsSyncAdapter);
            }
        } finally {
            c.close();
        }
    ...
    */
    ...
    public int deleteRawContact(long rawContactId, long contactId, boolean callerIsSyncAdapter) {
        ...
        /**
        * SPRD:
        *   modify this sentence when merge from 4.1 to 4.3.
        *
        * Original Android code:
    ...
    private boolean rawContactIsLocal(long rawContactId) {
        ...
        Cursor c = db.query(Tables.RAW_CONTACTS, Projections.LITERAL_ONE,
                ...
                * @{
                 */
                RawContactsColumns.ACCOUNT_ID + " in " + Clauses.NOT_SYNCABLE_ACCOUNT_IDS,
                /**
                * @}
                */
    ...
    private int markRawContactAsDeleted(SQLiteDatabase db, long rawContactId,
        ...
        */
        if (!ContactsContract.isProfileId(rawContactId)) {
            ContactProxyManager.getInstance(getContext()).remove(rawContactId);
        }
        /**
        * @}
        */
    ...
    private int updateGroups(Uri uri, ContentValues originalValues, String selectionWithId,
        ...
        Log.d(TAG, "number=="+originalValues
						.getAsString(Groups.GROUP_NUMBER)+"zone="+originalValues
						.getAsString(Groups.GROUP_ZONE_ID));
        final SQLiteDatabase db = mDbHelper.get().getWritableDatabase();
        final ContactsDatabaseHelper dbHelper = mDbHelper.get();
		if (originalValues.containsKey(Groups.GROUP_DEFAULT)
				&& originalValues != null && !TextUtils.isEmpty((originalValues
						.getAsString(Groups.GROUP_NUMBER))) && !TextUtils.isEmpty((originalValues
								.getAsString(Groups.GROUP_ZONE_ID)))) {
			Log.d(TAG, "searching the contacts coming in");
			Cursor checkAllcursor = db.query(Tables.GROUPS, new String[] {
					Groups._ID, Groups.GROUP_DEFAULT, Groups.GROUP_DEFAULT,
					Groups.GROUP_CURRENT_MODE, Groups.GROUP_NUMBER, Groups.GROUP_ZONE_ID },
					Groups.DELETED + " =0" + " AND " + Groups.GROUP_DEFAULT
							+ " = " +String.valueOf(Groups.GROUP_DEFAULT_FLAG)+" AND "+Groups.GROUP_ZONE_ID+" = "+String.valueOf(originalValues
						.getAsString(Groups.GROUP_ZONE_ID)), null, null,
					null, null);
			String updateDefaultId = "";
			int currentZone=0;
			Log.d(TAG, "searching the contacts coming out");
			while (checkAllcursor.moveToNext()) {
				updateDefaultId = checkAllcursor.getString((checkAllcursor
						.getColumnIndex(Groups.GROUP_NUMBER)));
				
				currentZone = checkAllcursor.getInt((checkAllcursor
						.getColumnIndex(Groups.GROUP_ZONE_ID)));

				Log.d(TAG, "updateDefaultId==" + updateDefaultId+"currentZone="+currentZone);
				ContentValues cv = new ContentValues();
				cv.put(Groups.GROUP_DEFAULT, 0);
				int count = db.update(Tables.GROUPS, cv, Groups.GROUP_NUMBER
						+ " =?", new String[] { updateDefaultId });
				Log.d(TAG, "count==" + count);
			}
    			checkAllcursor.close();
			if (!TextUtils.isEmpty((originalValues
					.getAsString(Groups.GROUP_NUMBER)))) {

				int count = db.update(Tables.GROUPS, originalValues,
						Groups.GROUP_NUMBER + " =?"+" AND "+Groups.GROUP_ZONE_ID + " =?", new String []{originalValues
					.getAsString(Groups.GROUP_NUMBER), originalValues
					.getAsString(Groups.GROUP_ZONE_ID)});
				if (originalValues.getAsString(Groups.GROUP_NUMBER) != null) {
					Log.d(TAG, "values.getAsString(Groups.GROUP_ID)=="
							+ originalValues.getAsString(Groups.GROUP_NUMBER)
							+ "count=" + count+"zone="+originalValues.getAsString(Groups.GROUP_ZONE_ID));
				}
				int zone_id=originalValues.getAsInteger(Groups.GROUP_ZONE_ID);
				if (originalValues.getAsLong(Groups.GROUP_CURRENT_MODE) == 0) {
					switch (zone_id) {
					case 0:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_1,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 1:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_2,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 2:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_3,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 3:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_4,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 4:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_5,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 5:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_6,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 6:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_7,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 7:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_8,
								originalValues.getAsString(Groups.GROUP_NUMBER));
					case 8:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_9,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 9:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_10,
								originalValues.getAsString(Groups.GROUP_NUMBER));
					case 10:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_11,
								originalValues.getAsString(Groups.GROUP_NUMBER));
					case 11:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_12,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 12:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_13,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 13:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_14,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 14:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_15,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 15:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_16,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					...
					case 16:
						...
						break;
					case 17:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_17,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 18:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_18,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 19:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_19,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 20:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_20,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;
					case 21:
						setPreferredDefaultGroup(PDT_GROUP_ZONE_21,
								originalValues.getAsString(Groups.GROUP_NUMBER));
						break;

					default:
						Log.d(TAG, "the send group zone id is error");
					}

				} else if (originalValues.getAsLong(Groups.GROUP_CURRENT_MODE) == 2) {
					setPreferredDefaultGroup(PREF_FILE_MPT,
							originalValues.getAsString(Groups.GROUP_NUMBER));
				}
			}
			return 0;
        ...
        */
        ...
        try {
            while (cursor.moveToNext()) {
                ...
                ContactProxyManager.getInstance(getContext())
                        .updateGroup(groupRowId, updatedValues);
            }
        } finally {
            cursor.close();
        }
    ...
    /**
     * add by xuhong.tian  remove the sync account operation 
    ...
    private boolean updateAccountsInBackground(Account[] systemAccounts) {
        ...
        try {
            ...
            */
            if (DEBUG){
                Log.d(TAG, "knownAccountsWithDataSets :" + knownAccountsWithDataSets);
                Log.d(TAG, "systemAccounts :\n");
                for (Account systemAccount : systemAccounts) {
                    Log.d(TAG, " system account -- " + systemAccount);
                }
                Log.d(TAG, "accountsWithDataSetsToDelete :" + accountsWithDataSetsToDelete);
                }
            /**
            * @}
            */
...
/*                for (AccountWithDataSet accountWithDataSet : accountsWithDataSetsToDelete) {
        ...
        /**
        * SPRD:
        *   modify these sentences when merge from 4.1 to 4.3.
        *
        * Original Android code:
        *  finally {
        *  db.endTransaction();
...
//                        maxSuggestions, filter, parameters);
    ...
    protected Account getDefaultAccount() {
        ...
        try {
            ...
            Log.e(TAG,
                    "Default account is empty, we should add the phone account");
    ...
    private int uniqueRawContacts() {
        ...
        Cursor cursor = db
                .rawQuery(
                        "select _id, contact_id from raw_contacts where deleted=0 and _id not in (select _id from raw_contacts where deleted=0 group by account_id, sync4 );"
                        , null
                );
        int ret = 0;
        try {
    ...
    private void setPreferredDefaultGroup(String currentZone,String defaultGroup) {
    	Log.d(TAG, "setPreferredDefaultGroup=="+defaultGroup+"currentZone=="+currentZone);
        SharedPreferences sp = getContext().getSharedPreferences(PREF_FILE_PDT, Context.MODE_WORLD_READABLE|Context.MODE_MULTI_PROCESS);
        SharedPreferences.Editor editor = sp.edit();
        editor.putString(currentZone, defaultGroup);
        editor.apply();
    }
